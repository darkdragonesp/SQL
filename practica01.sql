...
-- alfabético.
SELECT e.nombre, e.puesto, e.salario FROM curso.empleados e ORDER BY e.nombre ASC;
...
SELECT * FROM curso.empleados e WHERE YEAR(e.fechaAlta)=1981;
-- 9. Salario de los empleados, y salario resultante si se incrementa un 10%.
SELECT e.salario, e.salario+e.salario*0.1 AS sueldoMas FROM curso.empleados e;
-- 10.Nombre y departamento de los empleados sin comisión.
SELECT e.nombre,  d.nombre
	FROM curso.empleados e 
    LEFT JOIN curso.departamentos d ON e.dnumero=d.numero
...
SELECT e.nombre empleado, j.nombre jefe
	FROM curso.empleados e 
    LEFT JOIN curso.empleados j ON e.jefe=j.numero;
...
SELECT d.nombre, AVG(e.salario) media
